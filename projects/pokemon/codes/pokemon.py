...
class Poke:
    ...
    def get_atk(self, value):
        for atk in [self.atk1, self.atk2, self.atk3, self.atk4]:
            if atk.nombre == value:
                return atk
        ...
        if pr == 1:
            ...
            if("HEAL" in atk.efecto):
                ...
                if ((pk_att.vida + pk_att.vida_inicial*(tmp/100)) >= pk_att.vida_inicial):
                    ...
                    pk_att.vida += round(pk_att.vida_inicial*(tmp/100))
...
def enfrentamiento(logger, pk_r, pk_a, eff, atk_pk1, atk_pk2):
    if("HEALING" in pk_a.estado):
        ...
        if ((pk_a.vida + pk_a.vida_inicial/16) >= pk_a.vida_inicial):
            ...
            pk_a.vida += round(pk_a.vida_inicial/16)
    ...
    if("HEALING" in pk_r.estado):
        ...
        if ((pk_r.vida + pk_r.vida_inicial/16) >= pk_r.vida_inicial):
            ...
            pk_r.vida += round(pk_r.vida_inicial/16)
...
def main():
    ...
    if ((combat_type != "JvJ_c") & (combat_type != "JvC_c")):
        if os.path.exists('projects/pokemon/assets/session_' + id_session + '.log'):
            os.remove('projects/pokemon/assets/session_' + id_session + '.log')        
        file_handler = logging.FileHandler('projects/pokemon/assets/session_' + id_session + '.log', mode='a')
        ...
        if (combat_type != "CvC"):
            ...
            with open('projects/pokemon/assets/pok_' + id_session + '.pkl', 'wb') as f:
        ...
        file_handler.setFormatter(formatter)
        logger.addHandler(file_handler)
        with open('projects/pokemon/assets/pok_' + id_session + '.pkl', 'rb') as f:
